...
/-- **The drag stamps `hour ≤ h` under the per-hour confinement (PROVEN, left form).**  When the
Clock interactor `t` satisfies the per-hour confinement (its minute is below the boundary `(h+1)·K`,
the content of `ClocksBelowHour h`), the Rule-2 drag stamps the Main's hour `≤ h`.  This packages the
Main×Clock step in the per-hour-event idiom: a Clock drawn from a `ClocksBelowHour h` config supplies
the bound `hbound` directly. -/
theorem dragLeft_mainHour_le_of_clocksBelow {h : ℕ}
  ...
  PositionalCluster.dragLeft_mainHour_le s t hsM hsz htC (hconf t htmem htC)

/-- **The drag stamps `hour ≤ h` under the per-hour confinement (PROVEN, right form).** -/
theorem dragRight_mainHour_le_of_clocksBelow {h : ℕ}
  ...
  PositionalCluster.dragRight_mainHour_le s t hsC htM htz (hconf s hsmem hsC)
