...
/-- If a finite realized schedule from a valid-initial reachable configuration
remains in Phases 0--1 at its endpoint, then the endpoint's small-bias sum is
the original initial input gap. -/
theorem validInitial_nonuniformRunPairs_smallBiasSum_eq_initialGap
    (init c : Config (AgentState L K))
    (pairs : List (AgentState L K × AgentState L K))
    (hvalid : validInitial init)
    (hreach : (NonuniformMajority L K).Reachable init c)
    (hphase : ∀ a ∈ nonuniformRunPairs L K c pairs, a.phase.val ≤ 1) :
    smallBiasSum (nonuniformRunPairs L K c pairs) = initialGap init :=
