...
/-- If a finite stochastic support trace from a valid-initial reachable
configuration remains in Phases 0--1 at its endpoint, then the endpoint's
small-bias sum is the original initial input gap. -/
theorem validInitial_nonuniformSupportTrace_smallBiasSum_eq_initialGap
    (init c : Config (AgentState L K))
    (trace : List (Config (AgentState L K)))
    ...
    (htrace : nonuniformSupportTrace L K c trace)
    (hphase :
      ∀ a ∈ nonuniformSupportTraceEndpoint L K c trace, a.phase.val ≤ 1) :
    smallBiasSum (nonuniformSupportTraceEndpoint L K c trace) =
      initialGap init :=
  reachable_smallBiasSum_eq_initialGap (L := L) (K := K) init
