...
* **Item 1 is a BRIDGE with a named minimal missing export.**  `AllBiasedMainBelow top` (a snapshot:
  ...
  over the chain), and (b) `MainHourBelow top c` (provenance: `HourCouplingV2.Window` / the clock-front
...
* **Item 2 is a CORRECTED SCOPING (the honest verdict).**  `Phase3MainMainWindow` (all agents Main)
  ...
  chosen is that excluding clocks kills the **Phase-3 Rule-2 Main-Clock hour-drag**, which re-stamps
  an unbiased Main's `hour := min L (clock.minute / K)`.  That drag IS a genuine fresh `Z_i` supply
  source *inside* a window containing clocks: if `min L (clock.minute / K) > i` it pushes a `.zero`
  from `hour ≤ i` to `hour > i`, i.e. produces a fresh `supplyP i` agent.  So the honest verdict is:
  **on a mixed window (Main + Clock) the supply region needs the drag-control, and the drag-control IS
  ...
  (every Clock's `min L (minute / K) ≤ i`, so the drag cannot lift a zero above `i`), PROVE the
  Phase-3 Main-Clock drag is supply-neutral under it
  (`phase3_mainClock_drag_supplyP_neutral`), and assemble the corrected mixed-window
...
minimal missing clock export #2 (provenance: `HourCouplingV2.Window` / the clock-front
...
clock-front bound that controls the Phase-3 Rule-2 hour-drag: the drag re-stamps a Main's
`hour := min L (clock.minute / K)`, so under this bound the drag CANNOT lift a `.zero` to `hour > i`.
...
/-- **The Phase-3 Main-Clock drag fires to `(s', t)` with `s'.hour ≤ i` (PROVEN dispatch readout).**
When `s` is an unbiased Main and `t` is a Clock, `Phase3Transition L K s t` returns
`({s with hour := ⟨min L (t.minute/K), _⟩}, t)`: the drag re-stamps the Main's hour, the Clock is
unchanged, and the final both-Main branch does not fire.  This is the field-level honest content of
the audit-table "Main-Clock hour-drag" row. -/
theorem phase3Transition_mainClock_eq (s t : AgentState L K)
    (hsM : s.role = Role.main) (hsz : s.bias = Bias.zero) (htC : t.role = Role.clock) :
    ∃ h : min L (t.minute.val / K) < L + 1,
      Phase3Transition L K s t
        = ({ s with hour := ⟨min L (t.minute.val / K), h⟩ }, t) := by
  ...
  refine ⟨Nat.lt_succ_of_le (Nat.min_le_left _ _), ?_⟩
  unfold Phase3Transition
  simp only [hs_not_clock, if_false, if_pos ht_main_clock]
  -- s2 = { s with hour := … } (Rule-2 fires on s), t2 = t (the `else if` for t doesn't fire since
  -- the first guard already matched), final both-Main if is false.
...
the drag re-stamps the Main's `hour ≤ i`, so the output Main is NOT `supplyP i` (its hour is now
`≤ i`); the Clock output equals the Clock input (its supply status is preserved, never freshly
created).  Hence the supply count does not grow — the drag produces NO fresh `Z_i` supply when the
clock front is below `i`.  This is exactly the clock-front control of item 1 applied to item 2's
SEPARATE source. -/
theorem phase3_mainClock_drag_supplyP_subadditive (i : ℕ) (s t : AgentState L K)
  ...
  rw [heq]
  refine ⟨?_, rfl⟩
  -- output .1 = { s with hour := ⟨min L (t.minute/K), _⟩ }: bias = .zero, hour.val ≤ i.
  rw [supplyP]
  rintro ⟨-, hlt⟩
  -- hlt : i < (⟨min L (t.minute.val / K), h⟩ : Fin _).val = min L (t.minute.val / K);
  -- contradicts the drag bound `min L (…) ≤ i`.
  simp only at hlt
  omega
...
is the clock-front hour ceiling (item 1).  We record the verdict as the supply-neutrality of BOTH
the Main-Main and Main-Clock branches under the combined control. -/
...
(`MainClockDragBounded`), BOTH Phase-3 supply sources are neutralised:
