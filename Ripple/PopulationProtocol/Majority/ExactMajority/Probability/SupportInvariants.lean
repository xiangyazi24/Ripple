...
/-- At every finite Markov time from a valid initial configuration, any
endpoint that is still in Phases 0--1 has small-bias sum equal to the original
input gap. -/
theorem validInitial_nonuniformTransitionKernel_pow_phase_le_one_smallBias_eq_initialGap
    (init : Config (AgentState L K)) (hvalid : validInitial init) (t : ℕ) :
    ∀ᵐ c' ∂((nonuniformTransitionKernel L K ^ t) init),
      (∀ a ∈ c', a.phase.val ≤ 1) → smallBiasSum c' = initialGap init := by
  have hreach := ae_nonuniformReachable_transitionKernel_pow (L := L) (K := K) init t
  exact hreach.mono (by
    intro c' hreach' hphase
    exact reachable_smallBiasSum_eq_initialGap (L := L) (K := K)
      init c' hvalid hreach' hphase)

/-- The finite-time event that a Phase-0/1 endpoint has the wrong small-bias
sum has probability zero from any valid initial configuration. -/
theorem validInitial_nonuniformTransitionKernel_pow_phase_le_one_smallBias_ne_eq_zero
    (init : Config (AgentState L K)) (hvalid : validInitial init) (t : ℕ) :
    (nonuniformTransitionKernel L K ^ t) init
        {c' : Config (AgentState L K) |
          (∀ a ∈ c', a.phase.val ≤ 1) ∧ smallBiasSum c' ≠ initialGap init} = 0 := by
  have h :=
  ...
  simpa [Classical.not_imp] using h
