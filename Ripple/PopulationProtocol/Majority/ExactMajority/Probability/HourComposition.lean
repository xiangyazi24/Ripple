...
This file wires Lemma 6.10 (`HourCouplingV2.hour_coupling_v2`, the Azuma hour coupling) together
...
6.10 (`HourCouplingV2.hour_coupling_v2`) supplies the complementary fact: the MAIN agents' hour
...
counts `M`, `C`, `≥ 1` of each role — `HourCouplingV2.Regime`), the probability after `t ≥ 1`
...
theorem main_not_ahead_of_clock (M C : ℝ) (hM : 0 < M) (hC : 0 < C) (h : ℕ)
    ...
    (hreg : ∀ c : Config (AgentState L K),
      HourCouplingV2.Regime (L := L) (K := K) M C h c)
    (t : ℕ) (ht : 1 ≤ t) (c₀ : Config (AgentState L K)) {lam : ℝ} (hlam : 0 < lam) :
    ((NonuniformMajority L K).transitionKernel ^ t) c₀
        {c' | HourCouplingV2.Phi (L := L) (K := K) M C h c₀ + lam
            ≤ HourCouplingV2.Phi (L := L) (K := K) M C h c'}
      ≤ ENNReal.ofReal (Real.exp
          (-(lam ^ 2) / (2 * t * (2 / M + 2 * (11 / 10 : ℝ) / C) ^ 2))) :=
  HourCouplingV2.hour_coupling_v2 (L := L) (K := K) M C hM hC h hK hhL hreg t ht c₀ hlam
...
* **The burn-in IS the §6 width engine, already integrated into `εside`.**  The per-hour marked
  ...
  consumed by `EarlyDripMarked.windowedFrontProfile_whp_concrete` / `DotyParams.goodFrontWidth_whp_*`
