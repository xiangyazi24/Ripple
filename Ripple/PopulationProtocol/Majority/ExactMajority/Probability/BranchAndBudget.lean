...
`doty_expected_time_chain_end'` (`ChainEndRecut`) consumes: for every reachable not-done `b`,
...
`ReachableLadder.doty_expected_time_reachable'`), and condition the classification on the whp
