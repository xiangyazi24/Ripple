...
`DotyAssembly n`, but left its `work : Fin 11 → PhaseConvergenceW` field ABSTRACT — "supplied
...
## The 11-slot map (verified against `DotyTimeHeadline.lean:24` — "the eleven instances")
...
@[simp] theorem dotyWorkConcrete_one (wi : WorkInputs (L := L) (K := K) n) :
    dotyWorkConcrete wi ⟨1, by omega⟩
      = DrainCalibration.phase1Convergence_calibrated (L := L) (K := K) n wi.M₀ wi.t1
...
@[simp] theorem dotyWorkConcrete_six (wi : WorkInputs (L := L) (K := K) n) :
    dotyWorkConcrete wi ⟨6, by omega⟩
      = DrainCalibration.phase6Convergence_calibrated (L := L) (K := K) wi.l n wi.M₀ wi.q6 wi.tWin6
...
/-- **`dotyAssembly_concrete`** — a `ConcreteAssembly.DotyAssembly n` whose `work` field is the
...
noncomputable def dotyAssembly_concrete (wi : WorkInputs (L := L) (K := K) n)
    ...
    (hTrig : ∀ (k : Fin 10) (c : Config (AgentState L K)),
        (dotyWorkConcrete wi ⟨k.val, by omega⟩).Post c →
        SeamEpidemics.advTriggered (L := L) (K := K) (seamP k + 1) c)
    ...
    (hWindowToWorkPre : ∀ (k : Fin 10) (c : Config (AgentState L K)),
        ...
        (dotyWorkConcrete wi ⟨k.val + 1, by omega⟩).Pre c) :
...
`ConcreteAssembly` lemma — `dotyPhases`, the bridges, `doty_time_headline_CONCRETE` — sees the
concrete 11 slots). -/
@[simp] theorem dotyAssembly_concrete_work (wi : WorkInputs (L := L) (K := K) n)
    ...
    (dotyAssembly_concrete wi seamP seamT εepidemic εovershoot
