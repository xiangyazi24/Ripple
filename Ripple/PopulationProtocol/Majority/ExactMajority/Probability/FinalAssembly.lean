...
produced over a SINGLE inspectable hypothesis bundle `DotyResidualAtoms` — the definitive statement
of what genuinely remains (the irreducibly-probabilistic paper events).

## What this file delivers

1. `DotyResidualAtoms n L K C0` — the FINAL ATOM LIST as one Lean structure.  It bundles
   * `wi : AssemblyWiring.WorkInputs n` — the 11 WORK-slot probabilistic residual record (each field
     ...
     structural floors/closures/budget arithmetic already discharged inside `dotyWorkConcrete`;
...
2. `doty_theorem_3_1_whp` — the whp half.  `SeedTrigWiring.doty_time_headline_CONCRETE'`
   (the `hTrig`-free re-cut) instantiated with the assembly built from `DotyResidualAtoms`:
   conclusion `(K^T) c₀ {¬ stable} ≤ 21/n²` with `T ≤ 21·C0·n·(L+1)`, under `DotyRegime n L K`
   and a `Phase0Initial`-flavoured start.

3. `doty_theorem_3_1_expected` — the expectation half.  `ChainEndRecut.doty_expected_time_chain_end'`
   fed the SAME wired 21-instance family `dotyPhases'` and the on-chain branch classification
...
/-! ## Part 1 — `DotyResidualAtoms`: THE FINAL ATOM LIST.
...
re-cut headline `SeedTrigWiring.doty_time_headline_CONCRETE'`.
...
structure DotyResidualAtoms (n C0 : ℕ) where
  ----------------------------------------------------------------------------
  ...
  * `hConfine` (slot 5, Theorem 6.2 `arXiv:2106.10201v2`) — the `0.92·|M| ≤ #usefulMains`
  ...
  hWorkPostToWindow : ∀ (k : Fin 10) (c : Config (AgentState L K)),
      (AssemblyWiring.dotyWorkConcrete wi ⟨k.val, by omega⟩).Post c →
      SeamEpidemics.allPhaseGe (L := L) (K := K) (seamP k) n c
  ...
  hWindowToWorkPre : ∀ (k : Fin 10) (c : Config (AgentState L K)),
      ...
      (AssemblyWiring.dotyWorkConcrete wi ⟨k.val + 1, by omega⟩).Pre c
  ----------------------------------------------------------------------------
...
/-! ## Part 2 — the wired assembly `DotyAssembly'` built from the atoms. -/

/-- **The re-cut assembly built from `DotyResidualAtoms`.**  The work family is the wired
`AssemblyWiring.dotyWorkConcrete wi` (every WORK slot made concrete); the seam feeders / bridges /
one-step seed are the carried fields.  This is the `hTrig`-free `SeedTrigWiring.DotyAssembly'`. -/
noncomputable def toAssembly' {n C0 : ℕ} (ra : DotyResidualAtoms (L := L) (K := K) n C0) :
    SeedTrigWiring.DotyAssembly' (L := L) (K := K) n where
  work := AssemblyWiring.dotyWorkConcrete ra.wi
  seamP := ra.seamP
  seamT := ra.seamT
...
@[simp] theorem toAssembly'_work {n C0 : ℕ} (ra : DotyResidualAtoms (L := L) (K := K) n C0) :
    (toAssembly' ra).work = AssemblyWiring.dotyWorkConcrete ra.wi := rfl

/-- The wired 21-instance family of the final assembly (the SHIFTED-seam interleave). -/
noncomputable def phases' {n C0 : ℕ} (ra : DotyResidualAtoms (L := L) (K := K) n C0) :
    Fin 21 → PhaseConvergenceW (NonuniformMajority L K).transitionKernel :=
  SeedTrigWiring.dotyPhases' (toAssembly' ra)

/-! ## Part 3 — `doty_theorem_3_1_whp`: the whp half of Doty Theorem 3.1.

The re-cut headline `SeedTrigWiring.doty_time_headline_CONCRETE'` instantiated with the assembly
built from `DotyResidualAtoms`, under the paper regime `DotyRegime n L K`.  The conclusion is the
paper's Theorem 3.1 whp half: failure `≤ 21/n²` within `T ≤ 21·C0·n·(L+1)` interactions
(`= O(n log n)` interactions `= O(log n)` parallel time, since `DotyRegime` pins `L = ⌈log₂ n⌉`). -/
theorem doty_theorem_3_1_whp {n L K C0 : ℕ}
    (hReg : PaperRegime.DotyRegime n L K)
    (ra : DotyResidualAtoms (L := L) (K := K) n C0)
  ...
  -- clause CONSUMES `hReg` (`DotyRegime` pins `L = ⌈log₂ n⌉`), exhibiting the interaction bound as
  -- the paper's `O(n log n) = O(log n)` parallel-time form.
  obtain ⟨herr, htime⟩ :=
    SeedTrigWiring.doty_time_headline_CONCRETE' ra.init ra.c₀ (toAssembly' ra)
      ra.Cphase ra.δ T hT hcompFail ht hε hx₀ h_post ra.hC0 ra.hδ
  refine ⟨herr, htime, ?_⟩
  rwa [← hReg.hLlog]

/-! ## Part 4 — `doty_theorem_3_1_expected`: the expectation half of Doty Theorem 3.1.

`ChainEndRecut.doty_expected_time_chain_end'` fed the SAME wired family `phases' ra` and the on-chain
...
theorem doty_theorem_3_1_expected {n L K C0 Cbad Brecover : ℕ}
    (hReg : PaperRegime.DotyRegime n L K)
    (ra : DotyResidualAtoms (L := L) (K := K) n C0)
  ...
  -- (`DotyRegime` pins `L = ⌈log₂ n⌉`), exhibiting the expected hitting time as `O(n log n)`.
  have hcap := ChainEndRecut.doty_expected_time_chain_end' (L := L) (K := K) (n := n) (C0 := C0)
    ...
    ht hε (SeedTrigWiring.dotyPhases'_h_chain (toAssembly' ra)) hx₀ h_post ra.hC0
