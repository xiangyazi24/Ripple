...
produced the `WorkInputsV51.hpull1H` field through `PartnerMargin.EntrySumPinned`,
...
The V51 consumer field is `hpull1H : ∀ b, Phase1Honest n b → P1 ≤ pullPosSet.sum b.count`
...
/-! ## Stage C — the `pullPosSet` floor and the V51 `hpull1H` field producer.
...
/-- **Produces `WorkInputsV51.hpull1H` at the honest floor `P1 = (mc − g + 3) / 4`.**

Exact V51 field shape:
...
/-- **Split producer of `WorkInputsV51.hpull1H`.**  When the campaign carries the gap
...
/-- **Produces `WorkInputsV51.hext1H` (no all-Main bridge).**
