...
* **DotyTimeHeadline** — the seam-corrected 21-instance whp headline
...
* `Tgood = ∑ (phases i).t` is the good horizon and `δgood = ∑ δᵢ ≤ 1/n` the whp
  failure mass, both supplied by `doty_time_headline_W2`;
...
* `RecoveryClass`, `doty_recovery_expected_bound` — the recovery cap (§5).
* `doty_expected_time` — the top-level assembly against the real headline (§4.3).
* `doty_expected_time_concrete` — the concrete-constant corollary (§ arithmetic).
-/
...
protocol residual (see the file header). `doty_recovery_expected_bound` then turns a
...
Assembles `doty_time_headline_W2` (whp horizon + good time bound) with the recovery
...
* a recovery expected-time cap `hRecover` (built from `doty_recovery_expected_bound`)
...
theorem doty_expected_time_concrete
  ...
  · -- harith via the concrete closure
    have hclose := doty_harith_concrete n L C0 Cbad Brecover (2 * Brecover)
      hsRecCast hrecmass
    exact hclose
