...
1. **The hour-stamp ceiling `MainHourBelow (l+2)`** — every Main's `hour ≤ l+2`.  The hour FIELD of a
   ...
   * the **Phase-3 Rule-2 Main-Clock drag** (`HourCoupling.phase3_drag_left`/`_right`), which
     re-stamps `hour := min L (clock.minute / K)`;
...
The hour FIELD is written by the drag (`min L (minute/K)`) and the cancel (`hour := own index`).
...
clock-front content (the window `Window h`/`cAbove h = 0` confines the clocks below the current hour);
it is exactly the bound that makes the Rule-2 drag stamp `hour := min L (minute/K) ≤ h`. -/
...
`minute < (h+1)·K` with `0 < K`, then `min L (minute / K) ≤ h` — the Rule-2 drag stamp
`hour := min L (minute/K)` lands at hour `≤ h`.  This is the honest clock-front control of the
hour stamp. -/
...
/-- **The Rule-2 drag stamps the Main's hour `≤ h` (left form, PROVEN field readout).**  When `s` is
an unbiased Main and `t` is a Clock with `t.minute < (h+1)·K`, the drag output
`(Phase3Transition L K s t).1` has `hour.val = min L (t.minute/K) ≤ h`.  Honest content of the
...
theorem dragLeft_mainHour_le {h : ℕ} (s t : AgentState L K)
  ...
  exact dragStamp_le_of_clockBelow hbound L

/-- **The Rule-2 drag stamps the Main's hour `≤ h` (right form, PROVEN field readout).**  Symmetric:
when `s` is a Clock with `s.minute < (h+1)·K` and `t` is an unbiased Main, the drag output `.2` has
`hour.val = min L (s.minute/K) ≤ h`. -/
