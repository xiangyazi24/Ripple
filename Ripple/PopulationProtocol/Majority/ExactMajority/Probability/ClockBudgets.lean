...
The §6 concrete chain (`DotyParams.goodFrontWidth_whp_final`) delivers the moving-frame width
...
theorem widthFail_concrete (n : ℕ) (hn : DotyParams.N₀ ≤ n)
    ...
    (heB : ∀ T < Tcap,
      (GatedDrift.killK (markedK (L := L) (K := K) T (DotyParams.θn n))
          (taintedGate (L := L) (K := K) n) ^ (DotyParams.w n * DotyParams.KK L K))
          (some mc₀) {none} ≤ eB)
    (htB : ∀ T < Tcap,
      ENNReal.ofReal
        (Real.exp (DotyParams.σ (L := L) (K := K) n
          ...
          + 2 * DotyParams.σ (L := L) (K := K) n
              ...
              * ((DotyParams.w n * DotyParams.KK L K : ℕ) : ℝ)
          - DotyParams.σ (L := L) (K := K) n * ((DotyParams.tt n + 1 : ℕ) : ℝ))) ≤ tB)
    (W₂ : ℕ) (climbB : ℝ≥0∞)
    (hclimbB : (realκ L K ^ (DotyParams.w n * DotyParams.KK L K))
        (eraseConfig (L := L) (K := K) mc₀)
        {c | (c.card = n ∧ AllClockP3 (L := L) (K := K) c)
          ∧ ¬ ClimbBound (L := L) (K := K) (DotyParams.θ n) W₂ c} ≤ climbB) :
    (realκ L K ^ (DotyParams.w n * DotyParams.KK L K))
        (eraseConfig (L := L) (K := K) mc₀)
        {c | WidthSideP (L := L) (K := K) n c ∧
          ¬ GoodFrontWidth (L := L) (K := K) (FrontTail.frontWidthBound n + W₂) c}
      ≤ ((Tcap : ℝ≥0∞) * ((DotyParams.KK L K : ℝ≥0∞) * DotyParams.deltaB n + (eB + tB)))
          + climbB :=
  DotyParams.goodFrontWidth_whp_final (L := L) (K := K) n hn mc₀ hcard hge3 hnotP3 hclean
