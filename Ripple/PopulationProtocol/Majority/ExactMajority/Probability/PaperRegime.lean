...
## 2–4. The paper regime ties (`DotyRegime n L K`).

The headline `DotyTimeHeadline.doty_time_headline_W2` is polymorphic in `{L K n}`; the paper-regime
ties live only inside `DotyParams` (`N₀ ≤ n`) and the doctrine docs (`k = 45`, `L = ⌈log₂ n⌉`).
`DotyRegime n L K` collects them in ONE inspectable predicate:
...
* `hN : DotyParams.N₀ ≤ n`       — the finite-`n` floor (`10^40`) past every negligibility crossover
                                    of the §6 front engine (`DotyParams`, item 3 context).
...
parallel-time step fraction `DotyParams.w n = ⌊3n/200⌋` and the MGF window-rung ratio
`uW = 2(1+ε)·wp = 603/20000` (`DotyParams.uW`); it is never a drip rate in `Transition.lean`
...
auditor's suspicion: `wp` is an analysis threshold, file:line `DotyParams.lean:44` / `:454`.
...
/-! ## Part 4 — the paper regime ties `DotyRegime n L K` (items 2 & 4). -/
...
* `hN : DotyParams.N₀ ≤ n`       — the finite-`n` floor `n ≥ 10^40` past the §6 crossover.

`DotyTimeHeadline.doty_time_headline_W2` is stated polymorphically in `{L K n}`; threading
`DotyRegime n L K` as its regime hypothesis pins the concrete headline to the paper regime in ONE
place.  (`Nat.clog 2 n = ⌈log₂ n⌉` is Mathlib's ceiling-log.) -/
structure DotyRegime (n L K : ℕ) : Prop where
  /-- `L = ⌈log₂ n⌉` (item 4). -/
...
/-- `DotyRegime` pins `L = ⌈log₂ n⌉`, so `L + 1 = Θ(log n)` — the headline's `21·C0·n·(L+1)`
interaction bound is genuinely `O(n log n)` interactions `= O(log n)` parallel time. -/
theorem DotyRegime.L_eq_clog {n L K : ℕ} (h : DotyRegime n L K) : L = Nat.clog 2 n := h.hLlog

/-- `DotyRegime` pins `45 ≤ K` (the paper minutes/hour tie). -/
theorem DotyRegime.K_ge_45 {n L K : ℕ} (h : DotyRegime n L K) : 45 ≤ K := h.hK

/-- `DotyRegime` pins `N₀ ≤ n` (so every `DotyParams` discharger fires). -/
theorem DotyRegime.N₀_le {n L K : ℕ} (h : DotyRegime n L K) : DotyParams.N₀ ≤ n := h.hN

/-- Under `DotyRegime`, `n ≥ 10^40` so in particular `2 ≤ n` and `0 < n` (the headline's basic
size hypotheses are subsumed by the regime tie). -/
theorem DotyRegime.two_le_n {n L K : ℕ} (h : DotyRegime n L K) : 2 ≤ n :=
  DotyParams.two_le n h.hN

/-! ## Part 5 — `wp` is an analysis constant (item 3), recorded as a proven identity.

`DotyParams.uW = 2(1+ε)·wp = 603/20000` at `wp = 3/200`, `ε = 1/200`; `wp` enters ONLY this MGF
...
exhibits `wp = 3/200` purely as an MGF-analysis quantity (`DotyParams.uW`), never a transition
probability.  This is the closed form of the auditor's item-3 suspicion. -/
theorem wp_is_analysis_constant : DotyParams.uW = 2 * (1 + (1/200 : ℝ)) * (3/200 : ℝ) := by
  unfold DotyParams.uW; norm_num
