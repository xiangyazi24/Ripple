...
* **(2) `work2`/`work9` epidemic parameters.**  Survey claim: the chain's locked rationals
  (`DotyParams`) pin them.  **VERDICT: the union ALGEBRA is locked (arithmetic, `decide`); the
  epidemic SCALARS are NOT.**  `DotyParams` carries NO phase-2 opinion-union epidemic rate (no
...
the epidemic scalars `s,t,ε` (+ budget `hε`).  Survey verdict on the locked rationals: `DotyParams`
pins NO phase-2 opinion-union epidemic rate (`grep` over `DotyParams.lean` finds no `s`/`t`/`ε` for
...
fit `hε` are the honest scalar inputs (the survey finding: NOT pinned by `DotyParams`).  Both slot 2
...
reachability of every `Phase10Post` config (`hreach`), PRODUCE `AtomsV2.Phase10SignMatch init`.  The
...
`AtomsV2.postOfSign`: from the active all-phase-10 root and the chain reachability, any slot-20
...
theorem post_of_rooted {init : Config (AgentState L K)}
  ...
  AtomsV2.postOfSign (phase10SignMatch_of_rooted hinit hAllRoot hactRoot hreach) hPost
