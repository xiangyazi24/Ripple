...
The COMPOSITE headlines `DotyTimeHeadline.doty_time_headline_W2` and
`DotyExpectedTime.doty_expected_time`, however, only consumed the budget at the WEAKER
...
`1/n` enters purely at the union target in `doty_time_headline_W2`/`doty_expected_time`.
...
In `DotyExpectedTime.doty_expected_time` the good-horizon failure mass `δgood` enters
...
We re-instantiate `DotyTimeHeadline.doty_time_composition_W2` (the pure assembly arithmetic
...
/-- **`doty_time_headline_W2_tight` — the `O(1/n²)` seam-corrected composite headline.**

Identical to `DotyTimeHeadline.doty_time_headline_W2` except the union budget is taken at the
...
theorem doty_time_headline_W2_tight
    {L K n C0 C : ℕ}
...
/-- **`doty_time_headline_W2_inv_sq` — the composite at the realised constant `C = 21`.**
...
theorem doty_time_headline_W2_inv_sq
    {L K n C0 : ℕ}
    (init c₀ : Config (AgentState L K))
    (Cphase : Fin 21 → ℕ) (δ : Fin 21 → ℝ≥0)
    (phases : Fin 21 → PhaseConvergenceW (NonuniformMajority L K).transitionKernel)
  ...
  have h := doty_time_headline_W2_tight (C := 21)
...
`DotyExpectedTime.doty_expected_time` consumed `hδ : ∑ δ ≤ 1/n` and produced the recovery
...
/-- **`doty_expected_time_tight` — E[T] with the `C/n²` good-horizon budget.**

Identical conclusion to `DotyExpectedTime.doty_expected_time` (`E[T] ≤ Cexp·n·(L+1)`), but
...
rather than `(1/n)·sRecover·(1−1/2)⁻¹`; we route through the existing `doty_expected_time`
...
theorem doty_expected_time_tight
    {L K n C0 Cexp C : ℕ}
    (init c₀ : Config (AgentState L K))
    (Cphase : Fin 21 → ℕ) (δ : Fin 21 → ℝ≥0)
    (phases : Fin 21 → PhaseConvergenceW (NonuniformMajority L K).transitionKernel)
    ...
    expectedHitting (NonuniformMajority L K).transitionKernel c₀
      (StableDone L K init)
      ≤ ((Cexp * n * (L + 1) : ℕ) : ℝ≥0∞) := by
  -- relax the tighter `C/n²` budget to the `1/n` interface `doty_expected_time` expects.
  have hδ' : (∑ i, (δ i : ℝ≥0∞)) ≤ (1 / (n : ℝ≥0∞)) :=
    inv_sq_const_chain hn hC hδ
  -- `doty_expected_time` is stated with `1 / n` (= `1 / (n : ℝ≥0∞)`); align and apply.
  have hcast : (1 / n : ℝ≥0∞) = (1 / (n : ℝ≥0∞)) := by norm_num
  refine doty_expected_time
...
The previous `doty_expected_time` carried recovery `(1/n)·sRecover·(1−1/2)⁻¹ = 2·sRecover/n`.
