...
This file discharges that `sideB` at the concrete `DotyParams` parameters.
...
theorem frontSyncFail_concrete (n : ℕ) (hn : DotyParams.N₀ ≤ n)
  ...
  ClockFrontSyncFromWidth.frontSync_whp_of_goodFrontWidth (L := L) (K := K)
    (DotyParams.w n * j + r) (FrontTail.frontWidthBound n + W₂)
    (eraseConfig (L := L) (K := K) mc₀)
...
positive. -/
theorem w_pos_of_N₀ (n : ℕ) (hn : DotyParams.N₀ ≤ n) : 0 < DotyParams.w n := by
  unfold DotyParams.w
  have h200 : (200 : ℕ) ≤ 3 * n := by
    have : (10 : ℕ) ^ 40 ≤ n := by unfold DotyParams.N₀ at hn; exact hn
...
noncomputable def sideTerm (n : ℕ) (mc₀ : Config (MarkedAgent L K)) (Tcap W₂ B' : ℕ) (s : ℝ)
    (εP εB : ℕ → ℝ≥0∞) (τ : ℕ) : ℝ≥0∞ :=
  εWAt (L := L) (K := K) n mc₀ Tcap W₂ B' s (τ / DotyParams.w n) (τ % DotyParams.w n)
    + εP τ + εB τ
...
theorem frontSyncFail_at_free (n : ℕ) (hn : DotyParams.N₀ ≤ n)
    (mc₀ : Config (MarkedAgent L K))
    (hcard : mc₀.card = n)
    (hge3 : AllClockGE3 (L := L) (K := K) (eraseConfig (L := L) (K := K) mc₀))
    (hnotP3 : ¬ AllClockP3 (L := L) (K := K) (eraseConfig (L := L) (K := K) mc₀))
    ...
    (W₂ : ℕ) (hW₂ : 2 ≤ W₂) (B' : ℕ) (s : ℝ) (hs : 0 ≤ s)
    (εP εB : ℕ → ℝ≥0∞)
    (τ : ℕ) (hτ : τ < DotyParams.w n * DotyParams.KK L K)
  ...
  have hwpos : 0 < DotyParams.w n := w_pos_of_N₀ n hn
  -- the canonical decomposition.
  have hdecomp : DotyParams.w n * (τ / DotyParams.w n) + τ % DotyParams.w n = τ :=
    Nat.div_add_mod τ (DotyParams.w n)
  have hr : τ % DotyParams.w n < DotyParams.w n := Nat.mod_lt τ hwpos
  have hjKK : τ / DotyParams.w n ≤ DotyParams.KK L K - 1 := by
    have hlt : τ / DotyParams.w n < DotyParams.KK L K :=
  ...
  have key := frontSyncFail_concrete (L := L) (K := K) n hn mc₀ hcard hge3 hnotP3 hclean
    Tcap hcap W₂ hW₂ B' s hs (τ / DotyParams.w n) (τ % DotyParams.w n) hr hjKK (εP τ) (εB τ)
...
concrete `DotyParams` parameters from the gated start `erase mc₀`.
...
theorem sideB_concrete (n : ℕ) (hn : DotyParams.N₀ ≤ n)
    (mc₀ : Config (MarkedAgent L K))
    (hcard : mc₀.card = n)
    (hge3 : AllClockGE3 (L := L) (K := K) (eraseConfig (L := L) (K := K) mc₀))
    (hnotP3 : ¬ AllClockP3 (L := L) (K := K) (eraseConfig (L := L) (K := K) mc₀))
    ...
    (W₂ : ℕ) (hW₂ : 2 ≤ W₂) (B' : ℕ) (s : ℝ) (hs : 0 ≤ s)
    (εP εB : ℕ → ℝ≥0∞)
    (hP : ∀ τ < DotyParams.w n * DotyParams.KK L K,
      (ClockKilledMinute.realκ L K ^ τ)
        (eraseConfig (L := L) (K := K) mc₀)
        {c | ¬ ClockBudgets.WidthSideP (L := L) (K := K) n c} ≤ εP τ)
    (hbulk : ∀ τ < DotyParams.w n * DotyParams.KK L K,
      (ClockKilledMinute.realκ L K ^ τ)
        (eraseConfig (L := L) (K := K) mc₀)
        {c | ¬ (10 * rBeyond (L := L) (K := K)
            (ClockFrontShape.capMinute (L := L) (K := K)
              - (FrontTail.frontWidthBound n + W₂)) c < c.card)} ≤ εB τ) :
    ∑ τ ∈ Finset.range (DotyParams.w n * DotyParams.KK L K),
        ((NonuniformMajority L K).transitionKernel ^ τ)
          (eraseConfig (L := L) (K := K) mc₀) (HourSideBad (L := L) (K := K))
      ≤ εsync (L := L) (K := K) n mc₀ Tcap W₂ B' s εP εB
          (DotyParams.w n * DotyParams.KK L K) := by
  rw [εsync]
  refine Finset.sum_le_sum (fun τ hτ => ?_)
  have hτlt : τ < DotyParams.w n * DotyParams.KK L K := Finset.mem_range.mp hτ
...
`sideB_concrete`. -/
theorem heB_concrete (n T θn : ℕ) (hn : DotyParams.N₀ ≤ n)
    (mc₀ : Config (MarkedAgent L K))
    ...
    (GatedDrift.killK (markedK (L := L) (K := K) T θn)
        (taintedGate (L := L) (K := K) n)
          ^ (DotyParams.w n * DotyParams.KK L K)) (some mc₀) {(none : Option _)}
      ≤ εsync (L := L) (K := K) n mc₀ Tcap W₂ B' s εP εB
          (DotyParams.w n * DotyParams.KK L K) :=
  heB_of_sideB (L := L) (K := K) n T θn (DotyParams.w n * DotyParams.KK L K) mc₀ hx₀
    (εsync (L := L) (K := K) n mc₀ Tcap W₂ B' s εP εB (DotyParams.w n * DotyParams.KK L K))
...
theorem Sgood_compl_le_uniform (n mC T : ℕ) (hn : DotyParams.N₀ ≤ n)
    (mc₀ : Config (MarkedAgent L K))
    (hcard : mc₀.card = n)
    (hge3 : AllClockGE3 (L := L) (K := K) (eraseConfig (L := L) (K := K) mc₀))
    (hnotP3 : ¬ AllClockP3 (L := L) (K := K) (eraseConfig (L := L) (K := K) mc₀))
