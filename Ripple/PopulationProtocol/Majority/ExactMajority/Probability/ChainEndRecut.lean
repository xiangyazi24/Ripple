...
`hClass` (`ReachableLadder.lean:452-467`).  Yet `doty_expected_time_reachable`
(`ReachableLadder.lean:521-523`) and `doty_expected_time_chain_end`
...
misadvertises where floor data is consumed.
  ...
  → FIX (1): `doty_expected_time_reachable'` / `doty_expected_time_chain_end'`, re-cut WITHOUT
...
**FIX (3): the strongest re-cut `doty_expected_time` form** — `doty_expected_time_chain_end'`,
...
/-! ## Part D — FIX (1) + (3): the re-cut `doty_expected_time` surfaces (no dead `hFloors`)

These prove the SAME conclusion `E[T c₀ → StableDone] ≤ (21·C0 + 4·Cbad)·n·(L+1)` as
`ReachableLadder.doty_expected_time_reachable` / `ChainEndAssembly.doty_expected_time_chain_end`,
...
### Carried set of `doty_expected_time_chain_end'` (the strongest re-cut form)

| carried | role | honesty |
|---|---|---|
| 21-phase block (`phases`, `ht`, `hε`, `h_chain`, `hx₀`, `h_post`, `hC0`) | abstract whp headline composition (`doty_time_headline_W2`) | conditional-honest FRAGMENT (audit F5; unchanged) |
...
/-- **FIX (1) — `doty_expected_time_reachable'`, the de-deadweighted reachable-relative E4.**

`E[T c₀ → StableDone] ≤ (21·C0 + 4·Cbad)·n·(L+1)`.  Identical to
`ReachableLadder.doty_expected_time_reachable` but WITHOUT the dead `hFloors` binder (F6 (a)):
...
theorem doty_expected_time_reachable' {n C0 Cbad Brecover : ℕ}
  ...
  have hRecoverReach : ∀ b, ReachableFrom L K init b → b ∈ (StableDone L K init)ᶜ →
    ...
    doty_recovery_bound_via_ladder_on_reachable (n := n) init (Brecover : ℝ≥0∞)
...
open scoped Classical in
/-- **FIX (3) — `doty_expected_time_chain_end'`, the strongest re-cut form (no dead `hFloors`).**

`E[T c₀ → StableDone] ≤ (21·C0 + 4·Cbad)·n·(L+1)`.  Same conclusion as
`ChainEndAssembly.doty_expected_time_chain_end`, with the per-state classifier supplied as the
