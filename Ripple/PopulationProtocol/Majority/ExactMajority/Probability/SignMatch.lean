/-
# SignMatch — producing the conserved gap-sign-match residual `AtomsV2.Phase10SignMatch`.

This file is the *atom campaign first target*: it DISCHARGES the V3 surface's residual field
`hPhase10Sign : AtomsV2.Phase10SignMatch init` — the honest finding recorded in
`AtomsV2` (Part 2, F6 c) that `Phase10Post` does NOT pin the unanimous output `o` to the
init-gap sign.  The missing link is the conserved signed sum.

It is **append-only**: it edits NO existing file.  It imports `AtomsV2` (for `Phase10SignMatch`
...
`AtomsV2.Phase10SignMatch init` is `∀ c, Phase10Post c → phase10MajorityWitness init c`, where
...
`AtomsV2.Phase10SignMatch init`.  Then `AtomsV2.postOfSign` turns it into `h_post`, and the V3
...
the gap sign.  This is `AtomsV2.Phase10SignMatch` pointwise — the honest production from the
...
The whole-`init` production of `AtomsV2.Phase10SignMatch`, the field the V3 surface carries as
...
`AtomsV2.Phase10SignMatch init`.  This discharges the V3 `hPhase10Sign` residual into the
...
theorem phase10SignMatch_of_reachable {init : Config (AgentState L K)}
    ...
    (hact : ∀ c, Phase10Drop.Phase10Post (L := L) (K := K) c → hasActiveAgent c) :
    AtomsV2.Phase10SignMatch (L := L) (K := K) init := by
...
/-! ## Part 6 — wiring through `AtomsV2.postOfSign` to `h_post`.

The end-to-end check: the produced `Phase10SignMatch` feeds `AtomsV2.postOfSign` to turn any
...
/-- **`h_post` PRODUCED from conservation.**  Composing the campaign atom with `AtomsV2.postOfSign`:
...
theorem post_of_conservation {init : Config (AgentState L K)}
  ...
  AtomsV2.postOfSign (phase10SignMatch_of_conservation hcons hact) hPost
