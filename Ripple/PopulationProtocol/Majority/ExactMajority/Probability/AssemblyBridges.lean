...
# Assembly bridges — discharging the `DotyAssembly` structural-Pre gaps (`AssemblyBridges`)

This file attacks the three carried bridge fields of `ConcreteAssembly.DotyAssembly`
...
We show the `hWorkPostToWindow` field of `DotyAssembly` is DERIVABLE from per-phase
...
the exact field shape `ConcreteAssembly.DotyAssembly` requires.  Supplying `mk_…` instead of
...
composition output `.1` — unifying its kernel-power LHS `(K ^ ∑ (dotyPhases asm i).t) c₀ {…}`
against a restated copy — diverges.  `doty_time_headline_CONCRETE` carries it as the named
...
`(NonuniformMajority L K).transitionKernel ^ T` against `∑ i, (dotyPhases asm i).t` once `T`
...
the `Fin 21` sum shape.  This is exactly the shape `doty_time_headline_CONCRETE` already uses
...
Given the composition's failure-side output at the LITERAL sum horizon `∑ i, (dotyPhases asm
...
exactly the `hcompFail` hypothesis that `doty_time_headline_CONCRETE` consumes — produced,
not assumed. -/
